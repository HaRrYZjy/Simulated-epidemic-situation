...
- 玩家角色（蓝色）加入棋盘中央，可通过 `W/A/S/D` 或方向键移动。
- 玩家可以按 `空格` 释放“紧急治疗”，将周围一圈感染者转化为有抗体者。
- 新增“部署工具”系统：按 `1/2/3` 切换部署对象（医生/有抗体者/墙体），鼠标左键在空地放置。
- 部署需要消耗技能点，技能点会随时间自动恢复，鼓励玩家高频参与决策。
- 增加 `P` 暂停、`N` 单步推进，方便观察策略效果。
